import { BackgroundImage, BackgroundImageProps, Center, Text, TextProps } from '@mantine/core';
...
export const ImageStatesNameOptions = [
  ...
  { value: 'welcomeSign', label: 'Welcome to the System' },
];

export interface ImageStatesProps extends Omit<BackgroundImageProps, 'src' | 'alt'> {
  enable?: boolean;
...
const ImageStates = ({
  ...
  return (
    <Center style={{ width: '100%', display: 'flex', flexDirection: 'column' }}>
      <BackgroundImage src={Images[name]} w={width} h={height} {...props} />
      {message && (
        <Text size='lg' c='gray.5' mt={-12} fw={800} {...messageProps}>
          {message}
        </Text>
      )}
    </Center>
